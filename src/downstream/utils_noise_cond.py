...
def compute_auc(y_true, y_hat):
    if y_hat.shape[1] == 2:
        return roc_auc_score(y_true, y_hat[:, 1])
    else:
        return roc_auc_score(y_true, y_hat, average='macro', multi_class='ovr')
...
def bal_mixup_train(model, device, loss_fxn, optimizer, data_loader, history, epoch, model_dir, classes, mixup_alpha, wandb_log=False):
    ...
    for i, batch in pbar:
        ...
        bal_x = batch[1][0].to(device)
        bal_y = batch[1][1].to(device)

        lam = np.random.beta(mixup_alpha, mixup_alpha)
        mixed_x = (1 - lam) * x + lam * bal_x

        # Calculate sigma using EDM2 schedule
        sigma_min, sigma_max, rho = 0.002, 80, 7
        full_steps = 32
        step_indices = torch.randint(0, full_steps, (mixed_x.shape[0],), device=device, dtype=torch.float32)
        sigma = (sigma_max ** (1/rho) + step_indices / (full_steps - 1) * (sigma_min ** (1/rho) - sigma_max ** (1/rho))) ** rho
        
        out = model(mixed_x, sigma)
...
def train(model, device, loss_fxn, optimizer, data_loader, history, epoch, model_dir, classes, mixup, mixup_alpha, wandb_log=False):
    """Train PyTorch model for one epoch on NIH ChestXRay14 dataset.
    Parameters
    ----------
        model : PyTorch model
    ...
    for i, (x, y) in pbar:
        ...
        sigma_min, sigma_max, rho = 0.002, 80, 7
        full_steps = 32
        step_indices = torch.randint(0, full_steps, (x.shape[0],), device=device, dtype=torch.float32)
        sigma = (sigma_max ** (1/rho) + step_indices / (full_steps - 1) * (sigma_min ** (1/rho) - sigma_max ** (1/rho))) ** rho
...
def validate(model, device, loss_fxn, optimizer, data_loader, history, epoch, model_dir, early_stopping_dict, best_model_wts, classes, wandb_log=False):
    ...
    torch.save({'weights': model.state_dict(), 'optimizer': optimizer.state_dict()}, os.path.join(model_dir, 'latest.pt'))

    if wandb_log:
        import wandb
        wandb.log({
            "val/loss": running_loss / (i + 1),
            "val/balanced_acc": b_acc,
            "val/mcc": mcc,
            "val/auroc": auc,
            "epoch": epoch
        })
...
def evaluate(model, device, loss_fxn, dataset, split, batch_size, history, model_dir, weights, wandb_log=False):
    ...
    lb = LabelBinarizer()
    lb.fit(range(len(dataset.CLASSES)))
    transformed = lb.transform(y_true)
    if len(dataset.CLASSES) == 2:
        transformed = np.hstack((1 - transformed, transformed))
    
    true_df = pd.DataFrame(transformed, columns=dataset.CLASSES)
